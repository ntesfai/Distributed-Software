 ...
 * @license See above
 */
public class SampleAssign extends WaypointGUI implements
                                               ActionListener, ItemListener {
	
   private static final boolean debugOn = true;
   private WaypointLibrary wpLib;
   private String host = "localhost";
   private String port = "8080";
   private String url = "http://"+host+":"+port+"/";
   private WaypointCollectionHttpProxy wcp;
   
   public SampleAssign(String base, int argLen) {
      super(base);
      try{
      if(argLen >= 2) {
      ...
      wcp = new WaypointCollectionHttpProxy(new URL(url));
      }
      }catch(Exception ex){
   ...
   public void actionPerformed(ActionEvent e) {
	 ...
	 for(int i = 0; i < wpLib.wpts.size(); i++) {
	    if(key == wpLib.wpts.elementAt(i).name){
	        wpLib.wpts.remove(i); 
	        wpLib.wpts.elementAt(i);
		wpLib.obj.remove(key);
	    }
	 }
	 ...
	 wcp.addWpt(lat, lon, ele, name, addr);
	 ...
	 name = (String)frWps.getSelectedItem();
	 String nuName = nameIn.getText();
	 addr = addrIn.getText();
	 lat = Double.parseDouble(latIn.getText());
	 lon = Double.parseDouble(lonIn.getText());
	 ele = Double.parseDouble(eleIn.getText());
	 JSONObject ret = wcp.modWpt(lat, lon, ele, nuName, name, addr);
	 //JSONObject ret = wpLib.modLib(name, lat, lon, ele, addr);
         wpLib.obj.remove(name);
         wpLib.obj.put(name, ret);
         JSONObject obj = wpLib.obj.getJSONObject((String) frWps.getSelectedItem());
         nameIn.setText(obj.getString("name")); 
         addrIn.setText(obj.getString("address")); 
         latIn.setText(String.valueOf(obj.getDouble("lat"))); 
   ...
   public static void main(String args[]) {
      try{
         ...
         String name = "Ser321 HW5";
         if (args.length >= 2	) {
            name = args[0];
            SampleAssign sa2 = new SampleAssign(name, args.length);
         }
