...
bool WaypointLibrary::removeWpt(string wpt) {
    bool ret = false;
    if(waypoints.count(wpt) > 0){
        waypoints.erase(wpt);
	ret = true;
    }
    return ret;
...
bool WaypointLibrary::addWpt(double lat, double lon, double ele, string name, string address) {
    bool ret = false;
    Waypoint *temp = new Waypoint(lat, lon, ele, name, address);
    //waypoints[name] = *temp;
...
Json::Value WaypointLibrary::impjson_file() {
...
Json::Value WaypointLibrary::modWpt(double lat, double lon, double ele, string nuName, string name, string address) {
...
bool WaypointLibrary::expjson_file() {
    Json::Value jsonLib;
    jsonLib["name"] = name;
